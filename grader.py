...
def _contains_eval_token(text: str, token: str) -> bool:
    ...
    if re.fullmatch(r"[a-z0-9_]+", token):
        ...
        return any(
            bool(re.search(rf"\b{re.escape(variant)}\b", text)) for variant in variants
        )
...
def score_eval_note_coverage(text: str, eval_notes: str) -> tuple[float, list[str]]:
    ...
    for clause in re.split(r"[.,;:]", eval_notes.lower()):
        ...
        for alternative in re.split(r"\bor\b", clause):
            tokens = [
                token
                for token in re.findall(r"[a-z][a-z0-9_+-]*", alternative)
    ...
    for tokens in requirements:
        matched_tokens = sum(
            1 for token in tokens if _contains_eval_token(normalized, token)
        )
...
def _bwrap_command(sandbox_dir: str) -> list[str]:
    ...
    if not bwrap:
        raise RuntimeError(
            "bwrap is required for coder execution; set GRADER_ALLOW_UNSANDBOXED=1 to opt out"
        )
...
def _run_command(
    cmd: list[str], payload: str, env: dict[str, str], timeout: int
...
def execute_code_safely(
    code: str, timeout: int = 15, reference_tests: str | None = None
) -> dict:
    ...
    result["reference_assertions_found"] = len(
        re.findall(r"\bassert\b", normalized_reference_tests)
    )
    result["reference_tests_found"] = len(
        re.findall(r"\bdef\s+test_\w+", normalized_reference_tests)
    )
    ...
    try:
        ...
        if (
            "bwrap is required" in str(e)
            or "GRADER_SANDBOX" in str(e)
            or "unsandboxed" in str(e)
        ):
...
def grade_planner(response: str, prompt_meta: dict) -> GradeResult:
    ...
    num_headers = len(
        re.findall(r"^#+\s|^[A-Z][A-Za-z\s]+:\s*$|^\*\*[^*]+\*\*", text, re.MULTILINE)
    )
    ...
    phase_refs = re.findall(
        r"\b(step\s*\d|phase\s*\d|stage\s*\d|first|then|next|finally|before|after|depends on|prerequisite|blocked by)\b",
        text,
        re.IGNORECASE,
    )
    ...
    estimates = re.findall(
        r"\b(\d+\s*(hours?|days?|weeks?|minutes?|ms|seconds?)|low|medium|high|critical)\s*(risk|effort|priority|impact|complexity)",
        text,
        re.IGNORECASE,
    )
    ...
    tradeoffs = re.findall(
        r"\b(tradeoff|trade-off|pros?\s*(?:and|\/)\s*cons?|advantage|disadvantage|however|alternatively|on the other hand|downside|upside|versus|vs\.?)\b",
        text,
        re.IGNORECASE,
    )
    ...
    drift_signals = re.findall(
        r"\b(by the way|unrelated|off topic|anyway|let me know|hope this helps|feel free)\b",
        text,
        re.IGNORECASE,
    )
    ...
    if word_count > 2000:
        ...
        meaningful = set(
            w.lower() for w in words if len(w) > 3 and not w.startswith("#")
        )
    ...
    if re.search(
        r"\b(as an ai|i am a language model|i cannot|i don\'t have access)\b",
        text,
        re.IGNORECASE,
    ):
        hall_score -= 0.15
    ...
    filler = re.findall(
        r"\b(it is important to note|it should be noted|it is worth mentioning|needless to say|as you know|obviously)\b",
        text,
        re.IGNORECASE,
    )
...
def grade_coder(response: str, prompt_meta: dict) -> GradeResult:
    ...
    if all_code and parseable >= 0.5:
        ...
        has_self_checks = bool(
            exec_result["assertions_found"] or exec_result["tests_found"]
        )
        has_reference_checks = bool(
            exec_result["reference_assertions_found"]
            or exec_result["reference_tests_found"]
        )
    ...
    for name in set(
        m for m in method_matches + func_sigs if m not in ("def", "class", "self")
    ):
        checks += 1
    ...
    if all_code:
        ...
        good_func_names = sum(
            1 for n in func_names if re.match(r"^[a-z_]\w*$", n) or n.startswith("__")
        )
        good_class_names = sum(1 for n in class_names if re.match(r"^[A-Z]\w*$", n))
        naming_ratio = (good_func_names + good_class_names) / max(
            len(func_names) + len(class_names), 1
        )
    ...
    if result.code_blocks == 0:
        ...
        if not re.search(
            r"\b(note that|please note|keep in mind|disclaimer)\b", text, re.IGNORECASE
        ):
            follows += 0.10
